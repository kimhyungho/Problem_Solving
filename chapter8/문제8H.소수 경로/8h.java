...
public class Main {
	...
	public static void testCase(int caseIndex) {
		...
		while(!bfsQueue.isEmpty()){
			...
			if(visited[current.value] == true) continue;
			...
			for(int i = 0; i < 4; i += 1) {
				for(int j = 0; j < 10; j += 1) {	
					int next = getNext(i, j, current.value);
					
					if(next >= 1000 && isPrime[next]) {
	...
	public static int getNext(int i, int j, int value) {
		int pow = (int) Math.pow(10, i);
		
		return ((value / (pow * 10)) * (pow * 10)) + (value % pow) + (j * pow);
	}
