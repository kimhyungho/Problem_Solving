...
public class Main {
	...
	public static ArrayList<Bomb> getRemovableOrders(int n, Bomb[] bombs) {
		...
		while(!removableBombs.isEmpty()) {
			Bomb removedBomb = removableBombs.poll();
			removedBomb.remove();
			removedList.add(removedBomb);
			
			for(Bomb parent: removedBomb.getParentBombs()) {
				if(parent.getChildCount() == 0) {
					removableBombs.add(parent);
				}
			}
		}
